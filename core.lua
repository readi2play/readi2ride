...
R2R.Colors = READI.Helper.table:Merge(CopyTable(READI.Colors),
                                      {r2r = "d4aa00", r2r_light = "ffe680"})
R2R.data = {["addon"] = R2R.title, ["prefix"] = "R2R", ["colors"] = R2R.Colors}
--[[----------------------------------------------------------------------------
TEXTURES
----------------------------------------------------------------------------]] --
...
function R2R:OnEvent(evt, ...) if self[evt] then self[evt](self, evt, ...) end end
...
function R2R:FIRST_FRAME_RENDERED(evt)
    local mountTypeTitles = {
        R2R.L["Grounded"], R2R.L["Flying"], R2R.L["Aquatic"], R2R.L["Skyriding"]
    }
    for i = 1, #RD.MOUNT_TYPES do
        ...
        table.insert(R2R.mountLists[i].mounts, 1,
                     {txt = R2R.L["Unselect Mount"], val = ""})
    end
...
function R2R:GetContinent(zoneID)
    ...
    if info.mapType == 2 then _r = C_Map.GetMapInfo(info.mapID) end
...
function R2R:GetFilteredListOfMounts(filter, ...)
    ...
    if __addFlts > 0 then
        for i = 1, __addFlts do table.insert(filters, select(i, ...)) end
    end
    --[[
    ...
    local showCollected = C_MountJournal.GetCollectedFilterSetting(
                              RD.MOUNT_COLLECTED)
    local showNotCollected = C_MountJournal.GetCollectedFilterSetting(
                                 RD.MOUNT_NOT_COLLECTED)
    local showUnusable = C_MountJournal.GetCollectedFilterSetting(
                             RD.MOUNT_UNUSABLE)
...
function R2R:GetMountID()
    ...
    local currentContinent = RD.Helper.table:Filter(R2R.db.continents, function(c)
        if type(c.zoneID) == "table" then
            return READI.Helper.table:Contains(R2R.ContinentID, c.zoneID)
        end
    ...
    if R2R:IsMount(id) then
        ...
        isInstantSpell = spellInfo and spellInfo.castTime == 0
    end

    if IsPlayerMoving() then
        if isUsable and isInstantSpell then return id end
    else
        if isUsable then return id end
    end
    
    if R2R.db.bindings.indoors ~= "" then return R2R.db.bindings.indoors end    
...
function R2R:GetMount(id)
    ...
    local name, spellID, icon, _, isUsable = C_MountJournal.GetMountInfoByID(id)
    return name, spellID, icon, isUsable
end
...
function R2R:InitializeDB()
    ...
    _G[dbName] = _G[dbName] or {use_profiles = false}
    ...
    if not next(_G[dbName].global) then
        ...
        _G[dbName].global = READI.Helper.table:Merge({},
                                                     CopyTable(R2R.defaults),
                                                     _G[dbName].global)
    end
    ...
    if _G[dbName].use_profiles then
        _G[dbName].chars[charName] = _G[dbName].chars[charName] or
                                         CopyTable(R2R.defaults)

        _G[dbName].chars[charName].assigned_profile =
            _G[dbName].chars[charName].assigned_profile or charName
        local _ap = _G[dbName].chars[charName].assigned_profile

        if _ap ~= "global" then
            _G[dbName].chars[_ap] = READI.Helper.table:Merge({}, CopyTable(
                                                                 R2R.defaults),
                                                             _G[dbName].chars[_ap])
        else
...
SlashCmdList.R2R = function(msg, editBox)
    ...
    if READI.Helper.table:Contains(msg, configKeywords) then
        ...
        _G[AddonName .. '_Options'](configKeywords[1])
    end
end
